...
def main():
    with Driver() as drv:
        # ランダムな16個の符号付き整数で埋めた配列を作る
        A_ref = np.random.randint(-2500, 2500, 16).astype("int32")
        B_ref = np.random.randint(-2500, 2500, 16).astype("int32")

        # params setting
        inp = drv.alloc((2, 16), dtype="int32")
        out = drv.alloc(16, dtype="int32")
